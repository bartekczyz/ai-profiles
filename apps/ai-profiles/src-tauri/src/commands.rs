...
use crate::accounts::{self, ProfileAccount};
...
/// The account profile `id` is signed in under, read from what its CLI keeps
/// on disk. `None` when nothing there names one.
#[tauri::command(async)]
pub fn profile_account(id: String) -> AppResult<Option<ProfileAccount>> {
    accounts::read(&id)
}
