...
use super::instance::desktop_pid;
use super::list::{unmovable_reason, Session, SessionKind, SessionState};
...
fn lock_is_fresh(codex_home: &Path, id: &str, now: SystemTime) -> bool {
    let lock = codex_home
        ...
        .join(format!("{id}.lock"));
    let Ok(written) = fs::metadata(lock).and_then(|metadata| metadata.modified()) else {
        return false;
    };
    written
